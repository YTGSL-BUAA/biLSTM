...
def vols_serialization(dict_s, sample_idx):
    # us images
    us_mat = serialize_single_vol(im_folder=dict_s['img_a_path'], file_sfix='.nii', idx=sample_idx,
                                  cube_D=dict_s['cube_D'], ita=dict_s['ita'], norm_fact=255.0)
    # print us_mat.shape
    # print np.unique(us_mat)
    # auxiliary images
    aux_mat = serialize_single_vol(im_folder=dict_s['img_b_path'], file_sfix='-label.nii', idx=sample_idx,
                                   cube_D=dict_s['cube_D'], ita=dict_s['ita'], norm_fact=3.0)
    ...
    # label images
    targ_mat = serialize_single_vol(im_folder=dict_s['seg_c_path'], file_sfix='_seg.nii', idx=sample_idx,
                                    cube_D=dict_s['cube_D'], ita=dict_s['ita'], norm_fact=1)
...
def vols_serialization_bbx(dict_s, sample_idx):
    ...
    us_mat = serialize_single_vol(img_path=us_path, cube_D=dict_s['cube_D'], ita=dict_s['ita'], bbx_loc=bbx_loc,
                                  norm_fact=255.0)
    # print np.unique(us_mat)
    # print us_mat.shape
    ...
    targ_mat = serialize_single_vol(img_path=targ_path, cube_D=dict_s['cube_D'], ita=dict_s['ita'], bbx_loc=bbx_loc,
                                    norm_fact=1)
    targ_mat = targ_mat.astype('int')
    targ_shape = targ_mat.shape
    targ_entr_mat = np.zeros([targ_shape[0], targ_shape[1], 4]).astype('int')
...
def vols_serialization_subgrid(dict_s, sample_idx):
    ...
    #aux_c3_path = os.path.join(dict_s['img_bm_path'], (str(sample_idx) + "_aux_" + str(3) + ".mat"))
    #aux_c3_mat = serialize_single_vol(img_path=aux_c3_path, cube_D=dict_s['cube_D'], ita=dict_s['ita'],
                                      #bbx_loc=sub_grid_loc, norm_fact=255.0)
    # print np.unique(aux_mat)
    # print aux_mat.shape
    ...
    file_sfix = '.mat'
    us_path = os.path.join(dict_s['img_a_path'], (str(sample_idx) + file_sfix))
    us_mat = serialize_single_vol(img_path=us_path, cube_D=dict_s['cube_D'], ita=dict_s['ita'], bbx_loc=sub_grid_loc,
                                  norm_fact=255.0)
    # print np.unique(us_mat)
    # print us_mat.shape
    ...
    file_sfix = '_seg.mat'
    targ_path = os.path.join(dict_s['seg_c_path'], (str(sample_idx) + file_sfix))
    targ_mat = serialize_single_vol(img_path=targ_path, cube_D=dict_s['cube_D'], ita=dict_s['ita'],
                                    bbx_loc=sub_grid_loc, norm_fact=1)
    targ_mat = targ_mat.astype('int')
    targ_shape = targ_mat.shape
    targ_entr_mat = np.zeros([targ_shape[0], targ_shape[1], 2]).astype('int')
    ref_t = np.arange(targ_shape[1])
    for k in range(targ_shape[0]):
        targ_entr_mat[k, ref_t, targ_mat[k, :]] = 1  #这个函数用于将z坐标（几个class)的对应位置标记为1.例如，在targ_mat中，某个地方为2，则在对应(x,y,2)的地方被标记为1.
...
def serialize_single_vol(img_path, cube_D, ita, bbx_loc=None, norm_fact=255.0):
    ...
    vol_file = loadmat(img_path)
    vol_data = vol_file['data']
...
def serialize_single_vol_bbx(img_path, cube_D, ita, bbx_ext=10, norm_fact=255.0):
    # print '### loading sample from:\n %s' % (im_folder+ '/' + str(idx))
    vol_file = loadmat(img_path)
    vol_file = vol_file['data']
    vol_data = np.asarray(vol_file)
...
def deserialize_cubearray2grid(cube_array, grid_dim, cube_D, cube_ita):
    ...
    label_mat = np.stack((label_0_mat, label_1_mat), axis=3)
    # print label_mat.shape
    # print 'label mat unique:'
    # print np.unique(label_mat)
